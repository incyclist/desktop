...
pub async fn handle(cmd: Value, adapter: &Adapter, out: &IpcWriter) -> Result<()> {
    ...
    let mut obj = make_response("write", id.clone());
    let value = cmd.get("value").cloned();
    let without_response  = cmd.get("withoutResponse").cloned();
    let mut write_type = WriteType::WithoutResponse;

    // if withoutResponse is provided and true, set write_type to WithoutResponse
    if let Some(wr) = without_response {
        if wr.as_bool().unwrap()==false {
            write_type = WriteType::WithResponse;
        }
    }
        
    eprintln!("write: {:#?}, {:#?}",characteristic_uuid.as_deref().unwrap(),write_type);
    
    
    // get value as vec<u8>
    let data = value.unwrap().as_str().unwrap().as_bytes().to_vec();
        ...
        match find_peripheral_by_normalized(adapter, &norm).await {
            Ok(Some((peripheral, _))) => {
                ...
                if !peripheral.characteristics().is_empty() {
                    ...
                    match peripheral.write(target, &data,write_type).await {
                        Ok(_value) => {
