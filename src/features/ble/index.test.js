...
// close(): explicitly releases the native BLE manager (e.g. CoreBluetooth's
// CBCentralManager on macOS) instead of leaving it to the Noble binding's destructor
// during Node's exit-time cleanup - confirmed via stack sample (2026-08-09) that the
// latter can deadlock on macOS. Registered as a quit hook from register().
describe('BLEFeature — close', () => {
    ...
    test('calls stop() on the underlying Noble instance when present', () => {
        feature.ble = { stop: jest.fn() }

        feature.close()

        expect(feature.ble.stop).toHaveBeenCalled()
    })

    test('is a safe no-op when BLE was never initialized (e.g. Linux, where ble is null)', () => {
        feature.ble = null

        expect(() => feature.close()).not.toThrow()
    })

    test('is a safe no-op when the underlying binding has no stop() method', () => {
        feature.ble = {}

        expect(() => feature.close()).not.toThrow()
    })

    test('logs and swallows errors thrown by stop() rather than blocking quit', () => {
        feature.ble = { stop: jest.fn(() => { throw new Error('boom') }) }
        const logSpy = jest.spyOn(feature.logger, 'logEvent')

        expect(() => feature.close()).not.toThrow()
        expect(logSpy).toHaveBeenCalledWith(expect.objectContaining({message:'error', fn:'BLEFeature.close()'}))
    })
